...
## Step 2: Classify — decision, or note?

- **A real decision was made, with reasoning** → Step 3 (route to the existing Decisions Log).
- **Everything else worth sharing** → Step 4 (new Session Notes doc).
...
## Step 4: Non-decision findings get a Session Notes doc
...
**Content** (plain prose, no markdown syntax in the body text, formatting gets applied by the DOCX conversion in Step 5, not written by hand):
...
## Step 5: Build the doc via the confirmed-working pipeline

Do not write directly through a plain Drive-connector text upload, that path has a known markdown-escaping bug (literal `#`/`*`/`-` come out escaped instead of rendering as real formatting). Use the pipeline validated 2026-09-01 for `80_agents` docs:

1. Write the front matter + content (Steps above) to a local Markdown file in the scratchpad directory.
2. Convert it: `pandoc input.md -o output.docx`
3. Upload with Drive's own conversion: `import_to_google_doc` with `file_path` pointing at the `.docx`, `source_format: "docx"`, `folder_id` set to the `Session Notes` folder's ID, `file_name` set to the filename from Step 4.
...
## Step 6: Tell the user, don't just report success silently
...
- Never include specific dollar figures, margins, or anything marked "INTERNAL ONLY" in a Session Notes doc unless the project's own instructions already treat `80_agents` as strictly internal, same caution `team-meeting-debrief` applies to its Slack drafts.
