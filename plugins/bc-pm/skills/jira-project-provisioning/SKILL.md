...
**This applies just as much to a generic transport error as to a Jira error code.** A real OFS run (2026-09-23) returned "the connector's server isn't responding" — not a Jira error at all — yet `get_project_status` afterward showed every step had actually completed: the project, all three role assignments, and the board. The write had already landed server-side; only the response delivery failed. Don't treat "the connector's server isn't responding" as "nothing happened" just because it doesn't look like the usual Jira-error shape.

If a run fails (of either shape) or you're picking up a project someone else started, don't blindly re-run `provision_jira_project` — a retry will 400 on "project already exists" once the project itself was created, or worse, create a genuine duplicate board/filter if it gets far enough this time. Instead:

1. **Call `get_project_status`** with the project key — read-only, reports the project's current permission scheme, any boards it already has, and each scrum board's existing sprints (name + state). This tells you exactly what's left to do instead of reasoning it out from the original failure message. Call this first even on a transport-level error with no Jira detail at all — it's the only way to tell "never ran" from "ran fine, response just didn't arrive."
...
- **Transport-level failures are a distinct class from Jira API errors (2026-09-23):** the mcp-jira server had zero logging anywhere until a real OFS run returned a bare "connector's server isn't responding" with a fully-completed write underneath it — Netlify's Observability tab showed the request happened but not one log line about what it was doing when it "failed." Fixed by adding start/duration logging to every Jira API call and parallelizing `provision_jira_project`'s admin/member role-add loop (previously sequential, one full round trip at a time — the likelier a large admin/member list is, the more total request time scales). If this recurs, the logs now at least show which call was in flight and how long each step took.
