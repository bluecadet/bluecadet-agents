...
- **Fully root-caused 2026-09-22** (took several rounds — the full trail is worth knowing since it wasn't a single fix): classic OAuth scopes never covered the Jira Agile REST API at all — board/sprint/epic only exist as granular scopes under the "Jira Software" product (Atlassian does allow mixing classic scopes with granular ones across different products on the same app). Adding those granular scopes fixed board *creation* (`write:board-scope:jira-software` — one scope) but board *reads* (`get_project_status`, `get_board_by_id`) kept 401ing with a bodyless error even after a full connector disconnect/reconnect confirmed the right scopes were granted. Surfacing the `WWW-Authenticate`/error-body detail (rather than a bare status code) finally showed Atlassian's real reason: `"scope does not match"`. Checking Atlassian's own API docs directly (not guessing) showed board *read* endpoints require **two** scopes together — `read:board-scope:jira-software` **and** `read:project:jira` — plus `read:issue-details:jira` since board contents are issues. That's why writes worked immediately while reads kept failing through multiple reconnects: the missing scopes were never in what was being requested at all, no amount of reconnecting could have surfaced them. If a 401 shows up again, always get the tool's *exact* error text first (not just "401") — `jiraFetch` now surfaces both the response body and the `WWW-Authenticate` header, which is what actually broke this open after several rounds of guessing.
