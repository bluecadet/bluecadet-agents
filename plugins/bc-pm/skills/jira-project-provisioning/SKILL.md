...
- **Root-caused 2026-09-22:** classic OAuth scopes never covered the Jira Agile REST API at all — there's no classic equivalent for board/sprint/epic, they only exist as granular scopes under the "Jira Software" product (`read:board-scope:jira-software`, `write:board-scope:jira-software`, etc.). Fixed by adding those granular scopes to the connector's Atlassian app alongside the existing classic ones — confirmed Atlassian allows mixing classic and granular scopes across different products on the same app. If a 401 shows up again after this fix, it's something new — report the tool's *exact* error text (not just "401"), since `jiraFetch` forwards whatever detail Jira's response body includes.
