...
- [x] `team-slack-sync` (2026-09-24) — `team-meeting-debrief`'s sister skill: syncs a project's Slack channel(s) instead of a meeting transcript, for decisions/action items that only ever happened async in Slack. Shares the same `Decision Logging` and `Friction Logging` rule docs (`Global_Agents/Rules/`) `team-meeting-debrief` was just refactored (PR #28) to defer to — its first real second consumer. Hard-skips `team-meeting-debrief`'s own recap posts to avoid re-logging the same decision twice. No recap posted back to Slack, no standalone digest doc — confirmed decisions cite the source Slack message directly; action items are surfaced in-conversation only, nothing durable. Per-channel sync cursor lives in a new `80_agents/Slack Sync` doc.
